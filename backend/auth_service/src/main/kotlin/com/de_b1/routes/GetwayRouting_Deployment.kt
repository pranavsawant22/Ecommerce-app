...
@Serializable
@Resource("{param...}")
class GatewayRouting_Deployment(val parent: BaseRoute = BaseRoute())
...
fun Route.configureGatewayRouting_Deployment() {
    ...
    authenticate {
        ...
        post<GatewayRouting_Deployment> {
            println(call.request.uri)
            println(call.request.httpMethod)
            ...
            val lastName = principal!!.payload.getClaim("lastName").asString()
            val route = urlMap.get(call.parameters.getAll("param")?.get(0))
            val port = portMap.get(call.parameters.getAll("param")?.get(0))
            println("http://localhost:" + port + call.request.uri)
            val response: HttpResponse = httpClient.request(route + call.request.uri) {
        ...
        patch<GatewayRouting_Deployment> {
            println(call.request.uri)
            println(call.request.httpMethod)
            println(call.parameters.getAll("param")?.get(0))
            val entrymap = HashMap<String, String>()
...
//            print("hello"+response.bodyAsText())
        ...
        delete<GatewayRouting_Deployment> {
            println(call.request.uri)
            println(call.request.httpMethod)
            println(call.parameters.getAll("param")?.get(0))
            val entrymap = HashMap<String, String>()
